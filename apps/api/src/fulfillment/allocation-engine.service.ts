...
export class AllocationEngineService {
  ...
  private searchAtDepth(
    ...
    // Pruning: if the smallest denomination * remaining slots can't reach target, skip
    const remainingSlots = maxDepth - currentDepth;
    const minDenom = Math.min(...denominations.filter((_, i) => remaining[i] > 0).map((d) => d.faceValue), Infinity);
    if (minDenom === Infinity) return null;
