...
export class AdminController {
  ...
  async suggestProductSku(@Query('name') name: string, @Query('region') region: string) {
    return this.productsService.suggestSku(name || '', region || '');
  }
  ...
  async createManualOrder(@Body() body: {
    ...
    // The admin enters what to charge. A discount is still accepted so an older
    // client keeps working, and is turned into the same charge figure.
    const chargeAmount = body.chargeAmount !== undefined
      ? body.chargeAmount
      : body.discountAmount !== undefined
        ? Math.round((body.amount - body.discountAmount) * 100) / 100
        : undefined;
    // Only shape is checked. A manual sale is priced by hand, so the charge is
    // recorded as entered — there is no cap, and no comparison against the order
    // value that would need the two to share a currency.
