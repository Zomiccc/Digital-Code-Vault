  ...
  static normalize(headers: Record<string, any>, payload: any): NormalizedWebhookPayload {
    ...
    if (detected.provider === 'woocommerce') {
      ...
      return {
        ...
        platform: detected.platform,
        eventId: p.id || order?.id || p.event_id || null,
        orderId: String(order?.id || order?.order_id || '') || null,
        productId: String(lineItem?.product_id || '') || null,
        productName: lineItem?.name || lineItem?.title || null,
        productSku: lineItem?.sku || null,
        productCategory: lineItem?.categories?.[0]?.name || null,
        customerName: `${order?.billing?.first_name || ''} ${order?.billing?.last_name || ''}`.trim() || null,
        customerEmail: order?.billing?.email || null,
        quantity: lineItem?.quantity || 1,
        amount: Number(order?.total || order?.total_price || 0),
        currency: order?.currency || 'USD',
        paymentStatus: order?.status === 'completed' || order?.status === 'processing' ? 'paid' : order?.status || null,
