...
async function bootstrap() {
  ...
  const corsOrigins = configService.get<string>('CORS_ORIGIN', isDev ? '*' : 'http://localhost:5173,http://localhost:5174,http://localhost:5175');
  const isWildcard = corsOrigins.trim() === '*';
  app.use(cors({
